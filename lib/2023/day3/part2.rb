...
module Day3Part2
  def self.solution(lines)
    Schematic.parse_grid(lines).gear_ratios.sum
  end
  ...
  Schematic = Struct.new(:entities, :keyword_init => true) do
    ...
    def gear_ratios
      entities.select(&:star_label?).filter_map do |entity|
        adjacent_entities = _get_adjacent(entity)
        adjacent_parts = adjacent_entities.select(&:number_label?)

        next unless adjacent_parts.length == 2

        adjacent_parts[0].label.to_i * adjacent_parts[1].label.to_i
      end
    end
